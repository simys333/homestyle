    ...
    <div id="home">
       ...
       <LoadWhenVisible>
        ...
        <LazyHydrate when-visible>
            <SfBannerGrid :banner-grid="1" class="banner-grid">
                <template v-for="item in banners" #[item.slot]>
                    <SfBanner :key="item.slot" :title="item.title" :subtitle="item.subtitle" :description="item.description" :button-text="item.buttonText" image-tag="nuxt-img" :image="item.image" :nuxt-img-config="item.imageConfig" :class="item.class"/>
                </template>
            </SfBannerGrid>
        </LazyHydrate>
        ...
        <img else-if src="/homepage/banner-glass.png" />
         <LoadWhenVisible>
            <NewProducts class="products" :button-text="$t('View All ->')" :title="$t('Thoughtful Gifts for Every Kitchen Enthusiast !')" link="/what-is-new.html"/>
        </LoadWhenVisible>
        <LoadWhenVisible>
            <CallToAction :title="callToAction.title" :button-text="callToAction.buttonText" :description="callToAction.description" image-tag="nuxt-img" :image-src="callToAction.imageSrc" :image-width="callToAction.imageWidth" :image-height="callToAction.imageHeight" :nuxt-img-config="callToAction.imageConfig" class="call-to-action"/>
        </LoadWhenVisible>
...
export default defineComponent({
  ...
  setup() {
    ...
    const callToAction = ref({
      ...
      imageSrc: '/homepage/calltoaction.png',
      imageWidth: isDesktop ? 1320 : 400,
      imageHeight: isDesktop ? 172 : 200,
      imageConfig: {
        fit: 'contain',
        format: 'webp',
      },
...
.sf-call-to-action__image[data-v-54d0a06a] img  {
  object-fit:contain;
  }
}
